...
function UpdateCategory({ category, closeModal, onSuccess }) {
  ...
  useEffect(() => {
    if (category) {
      ...
      if (category.image) {
        setPreviewImage(process.env.REACT_APP_URL + category.image);
      }
    }
  }, [category]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await CategoryServices.updateCat(category.id, formData);
